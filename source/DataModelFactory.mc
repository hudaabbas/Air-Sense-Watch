...
class DataModelFactory {
    // Dependencies
    private var _delegate as BLEDelegate;
    ...
    public function initialize(delegate as BLEDelegate, profileManager as ProfileManager) 
    {
        _delegate = delegate;
    ...
    public function getEnvironmentModel(device as Device) as ProfileModel {
        var envModel = _envModel;
        if (envModel != null) {
            if (envModel.stillAlive()) {
                return (envModel.get() as ProfileModel);
            }
        }

        var dataModel = new $.ProfileModel(_delegate, _profileManager, device);
